...
class ValidationError(ZTAError):
    def __init__(self, message: str = "Invalid request", code: str = "INVALID_REQUEST") -> None:
        super().__init__(message=message, code=code, status_code=422)


class UnsafeOutputError(ZTAError):
    def __init__(self, message: str = "Unsafe model output detected", code: str = "UNSAFE_MODEL_OUTPUT") -> None:
        super().__init__(message=message, code=code, status_code=500)


class RateLimitError(ZTAError):
    def __init__(self, message: str = "Rate limit exceeded", code: str = "RATE_LIMIT_EXCEEDED") -> None:

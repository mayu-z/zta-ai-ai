...
class OutputGuard:
    def validate(self, template: str, real_identifiers: list[str]) -> None:
        if contains_raw_number(template):
            raise UnsafeOutputError(message="Template contains raw numeric values", code="RAW_VALUE_LEAK")

        lower_template = template.lower()
        for identifier in real_identifiers:
            if identifier and identifier in lower_template:
                raise UnsafeOutputError(message="Template leaked real schema identifier", code="SCHEMA_LEAK_DETECTED")

        for pattern in DISALLOWED_OUTPUT_PATTERNS:
            if pattern.search(template):
                raise UnsafeOutputError(message="Template includes unsafe system tokens", code="SYSTEM_TOKEN_LEAK")

        slots = re.findall(r"\[SLOT_\d+\]", template)
        if not slots:
            raise UnsafeOutputError(message="Template missing required SLOT placeholders", code="SLOT_MISSING")
